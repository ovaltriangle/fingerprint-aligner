...
private:
    /// constructor
    Reference reference;
    std::vector< std::pair<std::string, std::string> > query_seqs;
    FileManager query_manager, out_file, snps_file;
    char c_gap;
    std::size_t protein_threshold;
...
public:
    ...
    : reference(FileManager(reference_file).read_one_shot(), protein_threshold), query_manager(query_file), out_file(output_file), snps_file(snp_file),
      protein_threshold(protein_threshold), similarity_threshold(similarity_threshold), weights(weights)
    {
        /// Assign the gap character
        c_gap = query_manager.managed_file->c_gap;
        /// Read the query sequences
        query_seqs = query_manager.read_sequences();
